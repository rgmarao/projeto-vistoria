...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { data: perfil, error: errPerfil } = await supabase.from('perfis').select('*').eq('id', id).single();
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const perfilValido = ['admin', 'analista', 'usuario'];
    if (perfil && !perfilValido.includes(perfil)) return res.status(400).json({ ok: false, error: `Perfil inválido. Use: ${perfilValido.join(', ')}` });
    const { data, error } = await supabase.from('perfis').insert({ id, nome, perfil: perfil || 'usuario', ativo: true }).select().single();
    if (error) throw error;
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('perfis').update(campos).eq('id', id).select().single();
    if (error) throw error;
...
router.patch('/:id/ativar', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { data, error } = await supabase.from('perfis').update({ ativo: true, atualizado_em: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    if (!data) return res.status(404).json({ ok: false, error: 'Perfil não encontrado' });
    res.json({ ok: true, data });
  } catch (err) {
...
router.patch('/:id/desativar', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { data, error } = await supabase.from('perfis').update({ ativo: false, atualizado_em: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    if (!data) return res.status(404).json({ ok: false, error: 'Perfil não encontrado' });
    res.json({ ok: true, data });
  } catch (err) {
